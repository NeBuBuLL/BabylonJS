...
function createScene() {
    ...
    let followCamera = createFollowCamera(scene, zombie.zombieMesh.Zombie);
...
function createGround(scene) {
    const groundOptions = { width:2000, height:2000, subdivisions:40, minHeight:0, maxHeight:100, onReady: onGroundCreated};
...
function createZombie(scene){
    BABYLON.SceneLoader.ImportMesh("Ch30", "models/zomb/", "zombie.babylon", scene,  (newMeshes, particleSystems, skeletons) => {
        let zombieMesh = newMeshes[0];
        zombieMesh.position = new BABYLON.Vector3(0, 0, 5);
        zombieMesh.name = "zombie";

        let a = scene.beginAnimation(skeletons[0], 3, 241, true, 1);
        let zombie = new Zombie(zombieMesh, 1);
    });
